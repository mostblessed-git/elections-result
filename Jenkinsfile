pipeline {
    ...
    environment {
        DOCKERHUB_USERNAME = "mostblessed"
        DOCKER_IMAGE = "result"  // Change to your Docker Hub repo name
        DOCKER_TAG = "latest"
        DOCKER_HUB_CREDENTIALS = "docker-hub-credentials"  // Set this in Jenkins credentials
        EC2_INSTANCE = "ubuntu@15.223.70.12"  // Change to your EC2 public IP
    ...
    stages {
        stage('Checkout Code') {
            steps {
                git branch: 'main', url: 'https://github.com/mostblessed-git/elections-result.git'
            }
        }
                ...
                script {
                    sshagent(['ec2-ssh-key']) {
                        sh """
                            ...
                            docker stop result || true
                            docker rm result || true
    
                            echo "Running new container..."
                            docker run -d --name result --network front-tier --network back-tier -p 5001:80 $DOCKERHUB_USERNAME/$DOCKER_IMAGE:$DOCKER_TAG
EOF
                        """
                    }
                }
